...
- You must use search engines and external sources to find the latest- **Citations**: You MUST provide a source URL for every piece of information. **CRITICAL**: Prioritize article-specific deep links (e.g., `site.com/article-name`) over generic homepages. If you provide a homepage, you HAVE FAILED.
- **Deep Linking**: Never use shortened URLs or generic domain homepages.
- Focus on the following domains:
  1. **Stock Market**: Specific ticker symbols (defined in [tickers.md](file:///c:/Users/749534/Desktop/Daily-Report/config/tickers.md)) and general market sentiment.
     - **Premarket Data**: Since the report is for 9 AM, you MUST include premarket price action and sentiment.
